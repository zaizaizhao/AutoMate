...
import { getTestServerTools } from "../app/mcp-servers/mcp-client.js";
...
export async function ToolMessageExtract(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      console.log(
        `ToolMessageExtract attempt ${attempt + 1}/${maxRetries + 1}`
      );
...
7. 直接输出JSON，不要有任何解释性文字`;
      ...
      console.log(
        `ToolMessageExtract response (attempt ${attempt + 1}):`,
        response
      );
      ...
      if (response.content && typeof response.content === "string") {
        if (validateExtractedData(response.content)) {
          console.log(
            `ToolMessageExtract validation successful on attempt ${attempt + 1}`
          );
          validResponse = true;
          break;
        } else {
          ...
          console.warn(
            `ToolMessageExtract validation failed on attempt ${attempt + 1}:`,
            lastError
          );
        ...
        console.warn(
          `ToolMessageExtract response invalid on attempt ${attempt + 1}:`,
          lastError
        );
      ...
      console.error(
        `ToolMessageExtract error on attempt ${attempt + 1}:`,
        error
      );
  ...
  if (!validResponse) {
    console.error(
      `ToolMessageExtract所有重试都失败，使用默认格式。最后错误: ${lastError}`
    );
