  ...
  private setupToolHandlers() {
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      return {
        tools: [
          {
            name: 'writeJsonFile',
            description: 'Write structured data to a JSON file with support for append and overwrite modes',
            inputSchema: {
              type: 'object',
              properties: {
                filePath: {
                ...
                mode: {
                  ...
                  description: 'Write mode: append to existing data or overwrite the file'
                },
                createDirectories: {
                  type: 'boolean',
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      if (name === 'writeJsonFile') {
        return await this.handleWriteJsonFile(args as unknown as WriteJsonToolArgs);
      }
